"""
Module with gnirsarc2d arc-lamp methods.
...
# from pypeit.gnirsarc2d.wavecal import autoid
...
def fit2darc(all_wv, all_pix, all_orders, nspec, nspec_coeff=4, norder_coeff=4, sigrej=3.0, func2d='legendre2d',
             debug=False):
...
def resize_mask2arc(shape_arc, slitmask_orig):
    ...
    if nspec_orig != nspec:
        if ((nspec_orig > nspec) & (nspec_orig % nspec != 0)) | ((nspec > nspec_orig) & (nspec % nspec_orig != 0)):
            msgs.error(
                'Problem with images sizes. arcimg size and calibration size need to be integer multiples of each other')
        else:
            msgs.info(
                'Calibration images have different binning than the arcimg. Resizing calibs for arc spectrum extraction.')
...
def resize_slits2arc(shape_arc, shape_orig, trace_orig):
    ...
    if nspec_orig != nspec:
        ...
        trace = (scipy.interpolate.interp1d(spec_vec_orig, spat_ratio * trace_orig, axis=0, bounds_error=False,
                                            fill_value='extrapolate'))(spec_vec)
    else:
        trace = trace_orig
...
def resize_spec(spec_from, nspec_to):
    ...
    if nspec_from != nspec_to:
        ...
        spec_to = (
            scipy.interpolate.interp1d(spec_vec_from, spec_from, axis=0, bounds_error=False, fill_value='extrapolate'))(
            spec_vec_to)
    else:
        spec_to = spec_from
...
def iter_continuum(spec, inmask=None, fwhm=4.0, sigthresh=2.0, sigrej=3.0, niter_cont=3, cont_samp=30,
                   cont_frac_fwhm=1.0,
    ...
    for iter in range(niter_cont):
        ...
        if (frac_mask > 0.70):
            msgs.warn('Too many pixels masked in spectrum continuum definiton: frac_mask = {:5.3f}'.format(
                frac_mask) + ' . Not masking....')
